...
try {
  ...
  app.use(session({
    ...
    store,
    cookie: { httpOnly: true, secure: process.env.NODE_ENV === 'production', maxAge: 7 * 24 * 60 * 60 * 1000, sameSite: 'strict', domain: process.env.NODE_ENV === 'production' ? process.env.VERCEL_URL : undefined, path: '/' },
    name: 'theagencyiq.sid'
  }));
} catch (error) {
  console.warn('Session store failed, fallback to memory');
  app.use(session({
    secret: process.env.SESSION_SECRET,
    resave: false,
    saveUninitialized: false,
...
app.use(csurf({ cookie: true })); // FIXED: CSRF protection for security/GDPR/UE
