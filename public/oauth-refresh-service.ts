...
export class OAuthRefreshService {
  ...
  static async validateAndRefreshConnection(platform: string, userId: number): Promise<RefreshResult> {
    try {
      ...
      static async revoke(platform: string, userId: number) {
  const connection = await storage.getPlatformConnection(userId, platform);
  const revokeUrl = platformRevokeUrls[platform]; // FIXED: Define per platform from research (e.g., 'https://api.linkedin.com/v2/accessToken' for LinkedIn DELETE)
  await axios.delete(revokeUrl, { data: { access_token: connection.accessToken } });
